"""
方案3-v4：DFFormer + MoE 调优版本（第三版 - 最终调优）
核心原则：只调正则化超参，不动架构和损失函数。

基于 v2/v3 的教训：
- 损失函数形态不能改（否则尺度崩塌）
- LR 1e-5 是对的，5e-6 太保守
- dropout 0.1→0.2（轻微增强）
- weight_decay 1e-5→5e-5（轻微增强）
- warmup_epochs=2 防止冷启动跳步
- patience=8
...
class MoEConfigV4:
    ...
    ranking_weight = 1.0
    pairwise_weight = 0.5   # 与原版相同
    moe_weight = 0.01        # 与原版相同

    feature_num = '158+39'
    output_dir = '../model/df_former_v4_60_158+39'
    data_path = '../data'
...
# ============================================================
# 模型代码（与原版 train_df_former.py + model_df_former.py 完全一致，仅 dropout 增大）
# ============================================================
...
class MoELayer(nn.Module):
    def __init__(self, input_dim, num_experts=3, hidden_dim=128, output_dim=64, top_k=2, dropout=0.2):
        super().__init__()
...
class TemporalStream(nn.Module):
    def __init__(self, feature_dim, d_model, dropout=0.2):
        super().__init__()
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, writer, cfg):
    ...
    for batch in tqdm(dataloader, desc=f"[DFv4] Train E{epoch+1}"):
...
def evaluate_epoch(model, dataloader, criterion, device, epoch, writer, cfg):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"[DFv4] Eval E{epoch+1}"):
            sequences = batch['sequences'].to(device)
            targets = batch['targets'].to(device)
            masks = batch['masks'].to(device)
...
def main():
    ...
    with open(os.path.join(cfg.output_dir, 'config_df_former_v4.json'), 'w') as f:
    ...
    joblib.dump(scaler, os.path.join(cfg.output_dir, 'scaler_df_former_v4.pkl'))
    ...
    model.to(device)
    print(f"DFFormer+MoE V4 参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    ...
    for epoch in range(cfg.num_epochs):
        ...
        if current_metric > best_metric:
            ...
            best_epoch = epoch + 1
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(cfg.output_dir, 'best_model_df_former_v4.pth'))
    ...
    with open(os.path.join(cfg.output_dir, 'final_score_df_former_v4.txt'), 'w') as f:
